...
VM_URL="https://vmselect.test.com/select/0/prometheus"
...
REQUIRED_CMDS="jq curl sar mpstat zip nic_status.sh iostat ss ethtool"
...
echo "=== SAN Debug Collection v3.36 ==="
echo "  Device            : $DEV"
...
echo "[1/10] Capturing initial state..."
...
echo "[2/10] Discovering connected switches..."
nic_status.sh > "${PREFIX}_nic_status.log" 2>/dev/null || true

discover_switch_port() {
    local iface="$1"
    local info
    info=$(grep -A 20 "^$iface " "${PREFIX}_nic_status.log" | grep "Connected Switch" | head -n1 || true)
    if [[ -z "$info" ]]; then
    ...
    local switch_raw
    local port_raw
    switch_raw=$(echo "$info" | awk -F':' '{print $3}' | awk -F',' '{print $1}' | xargs)
    port_raw=$(echo "$info" | awk -F',' '{print $2}' | awk '{print $2}' | xargs)
    local switch_clean
    switch_clean=$(echo "$switch_raw" | cut -d'.' -f1)
    echo "$switch_clean $port_raw"
}
...
for iface in $IFACES; do
    read -r sw pt <<< "$(discover_switch_port "$iface")"
    SWITCH_MAP["$iface"]="$sw"
    PORT_MAP["$iface"]="$pt"
    echo "  - $iface -> Switch: $sw, Port: $pt"
done
...
echo "[8/10] Capturing final state..."
...
echo "[9/10] Querying VictoriaMetrics (Pure host & volume metrics)..."

# Helper: build weighted latency (lat * iops / iops) in ms
...
fetch_pure_vol_latency() {
    ...
    # If no volume defined, just skip
...
fetch_pure_host_iosize() {
    local DIM="$1"   # bytes_per_read / bytes_per_write
    local OUTFILE="$2"
...
#######################################
# SWITCH METRICS (VictoriaMetrics)
#######################################
...
fetch_switch_metric() {
    ...
    if [[ "$SW" == "unknown" || "$PT" == "unknown" || -z "$SW" || -z "$PT" ]]; then
        return
    fi

    echo "  - Querying Switch ${SW} Port ${PT} (${IFACE})..."

    local METRICS_REGEX="router_receive_errs_total|router_transmit_errs_total|router_receive_discard_packets_total|router_transmit_discard_packets_total|router_receive_fcs_errs_total"
    local Q_ERR="sum by (instance, intf, __name__) (increase({instance=~\"${SW}.*\", intf=\"${PT}\", __name__=~\"${METRICS_REGEX}\"}[5m]))"

    curl -k -f -s -G \
        --data-urlencode "query=${Q_ERR}" \
        --data-urlencode "time=${END_TS}" \
        "${VM_URL}/api/v1/query" \
        | jq '.' > "${PREFIX}_vm_switch_errors_${IFACE}.json" || true

    local BW_REGEX="router_receive_bytes_total|router_transmit_bytes_total"
    local Q_BW="sum by (instance, intf, __name__) (increase({instance=~\"${SW}.*\", intf=\"${PT}\", __name__=~\"${BW_REGEX}\"}[5m]))"

    curl -k -f -s -G \
        --data-urlencode "query=${Q_BW}" \
        --data-urlencode "time=${END_TS}" \
        "${VM_URL}/api/v1/query" \
        | jq '.' > "${PREFIX}_vm_switch_bytes_${IFACE}.json" || true
}

for iface in $IFACES; do
    fetch_switch_metric "$iface" "${SWITCH_MAP[$iface]}" "${PORT_MAP[$iface]}"
done
...
echo
echo "===== SAN DEBUG QUICK SUMMARY (v3.36) ====="
...
echo "[10/10] Compressing logs into ZIP..."
ZIP_FILE="${PREFIX}.zip"

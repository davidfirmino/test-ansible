#!/usr/bin/env python3
"""
collect_blockrq_raw.py

Drop-in-ish replacement for biosnoop for environments where biosnoop itself does
not emit events, but block raw tracepoints do. It outputs a biosnoop-like raw log
that the existing san_debug_analyzer.py can parse with minimal or no changes.

Key design:
- Uses raw block tracepoints instead of biosnoop's historical hook mix.
- Correlates block_rq_insert -> block_rq_issue -> block_rq_complete by request ptr.
- Prints a biosnoop-like stream:
    TIME(s) COMM PID DISK T SECTOR BYTES QUE(ms) LAT(ms)

Notes / caveats:
- QUE(ms) is reconstructed as insert->issue. In some fast/NVMe paths this may be
  0 or slightly different from classic biosnoop.
- PID/COMM are captured at insert when available, else at issue.
- Disk filtering is done in-kernel by disk_name string match (eg dm-8).

Requirements:
- root
- python3-bcc / bcc
- block raw tracepoints available
"""
...
BPF_HASH(start, struct request *, struct stage_t, 262144);
BPF_PERF_OUTPUT(events);

static __always_inline int str_eq_target(const char *name)
{
...
static __always_inline void fill_disk_name(struct request *rq, char *out)
{
...
static __always_inline int save_stage(struct request *rq, bool is_insert)
{
    struct stage_t zero = {};
    struct stage_t *st;
    ...
    if (!st) {
        ...
        fill_disk_name(rq, zero.disk_name);
        if (!str_eq_target(zero.disk_name))
            return 0;
    ...
    if (!st->disk_name[0])
        fill_disk_name(rq, st->disk_name);
...
RAW_TRACEPOINT_PROBE(block_rq_complete)
{
    ...
    ev.len = st->len;
    if (!ev.len)
        ev.len = rq->__data_len;
    ev.cmd_flags = st->cmd_flags ? st->cmd_flags : rq->cmd_flags;
    ev.pid = st->pid;
...
class Collector:
    ...
    def _handle_event(self, cpu: int, data: int, size: int) -> None:
        ...
        # biosnoop-like spacing; analyzer only needs split() compatibility
        print(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Collect biosnoop-like block I/O events using raw block tracepoints.")
    parser.add_argument("-d", "--device", required=True, help="disk name to trace, eg dm-8 or nvme0n1")
    parser.add_argument("--debug", action="store_true", help="enable basic debug mode")
    ...
    global collector
    collector = Collector(device=args.device, debug=args.debug)

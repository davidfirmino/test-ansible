#!/usr/bin/env python3
"""
collect_blockrq_tp.py

A biosnoop-compatible host I/O collector built on standard block tracepoints.

Design choices:
- Uses block:block_rq_issue and block:block_rq_complete as regular tracepoints.
- Does NOT use raw tracepoints.
- Does NOT depend on biosnoop.
- Produces a raw log that matches the existing analyzer input format:

  TIME(s) COMM PID DISK T SECTOR BYTES QUE(ms) LAT(ms)

Notes:
- QUE(ms) is emitted as 0.000 in this implementation.
- LAT(ms) is measured from block_rq_issue -> block_rq_complete.
- The collector filters in-kernel by the requested block device.
"""
